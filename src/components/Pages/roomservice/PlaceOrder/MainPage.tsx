  ...
  if (cart.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate({ to: "/room-service" })} className="text-[#6F5D29] underline">
          Go back to services
        </button>
      </div>
    );
    ...
    <div className="min-h-screen pb-28">
      <div className="max-w-md mx-auto px-4 pt-6 space-y-4">
        ...
        <div className="space-y-3 relative">
          ...
          {cart?.map((item: ServiceSchema & { quantity: number }) => (
            <div key={item.name} className="flex gap-3 items-center bg-[#FCFEFF] rounded-[20px] p-2 shadow-sm relative">
              {/* Image */}
              <img src={item.imageUrl[0]} alt={item.name} className="w-22 h-22 object-cover rounded-lg" />

              {/* Content */}
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-semibold text-[#3D3D3D] text-[14px]">{item.name}</div>
                    <div className="text-[12px] text-[#847777] line-clamp-1">{item.description || "Vegetables soup / fish cake.."}</div>
                  </div>
                  <button onClick={() => removeFromCart(item.name)} className="text-[16px] font-extrabold">
                    <X className="w-4 h-4 text-[#FF5252] font-extrabold" />
                  </button>
                </div>

                <div className="text-[#3D3D3D] text-[14px] font-bold">${item.price!.toFixed(2)}</div>
              </div>
              ...
              <div className="flex items-center gap-2 absolute right-3 bottom-1 -translate-y-1/2">
                <button onClick={() => updateQuantity(item.name, item.quantity - 1)} className="w-5 h-5 border rounded flex items-center justify-center">
                  <Minus size={14} />
                </button>
          ...
          <div className="bg-white rounded-[7.28px] border border-gray-200 shadow-sm p-2 flex items-center mb-15">
            ...
            <input type="text" placeholder="Special Request" className="flex-grow text-gray-800 placeholder-[#A89A81] focus:outline-none" />
          </div>
        </div>
      </div>
