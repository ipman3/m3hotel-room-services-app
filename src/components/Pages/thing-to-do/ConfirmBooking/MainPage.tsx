...
import { confirmBookingSchema, type ConfirmBookingInput } from "@/validations/confirmBooking";

export default function ConfirmBookingPage() {
  const navigate = useNavigate();
  const { items, confirmPendingItem } = useCartStore();

  const filteredItems = items.filter((item) => item.serviceType === "tourpackage");
  ...
  if (items.length === 0) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        ...
        <Button onClick={() => navigate({ to: "/wellness-spa" })} className="mt-4 bg-base-primary text-card">
          Go back to services
        </Button>
      </div>
    );
      ...
      <div className="p-4 bg-muted-background rounded-2xl customShadowSm">
        ...
        <div className="space-y-2">
          {filteredItems.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-2 pb-3 text-sm border-b border-gray-100 last:border-0 last:pb-0">
              <div>
                <img src={item?.image} alt={item?.name} className="flex-shrink-0 object-cover rounded-lg w-22 h-22" />
              </div>
              <div className="flex flex-col justify-between flex-grow">
                <div>
                  <h4 className="text-base font-semibold">{item.name}</h4>
                  <p className="text-sm text-muted-foreground">
                    {item.packageName} Package - {item?.category_id} Type
                  </p>
                  {item.date && (
                    <p className="text-sm text-muted-foreground">
                ...
                <div className="flex">
                  <span className="text-base font-semibold text-base-primary">Price: ${parseInt(item.price).toFixed(2)}</span>
                </div>
              </div>
            </div>
          ))}
